...
if(not os.path.isfile(fastqc_storage)) and("fastqc" in steps_to_process):
...
if(("samformatconverter" in steps_to_process) or ("all" in steps_to_process)):
...
sortsam_bam = picard_storage_dir+r1_base_filename + "_sorted.bam"
if (("sortsam" in steps_to_process) or ("all" in steps_to_process)):
...
if (("collecthsmetrics" in steps_to_process) or ("all" in steps_to_process)):
...
if (("collectismetrics" in steps_to_process) or ("alignmentqc" in steps_to_process)):
...
if (("collectalignmentsummarymetrics" in steps_to_process) or ("alignmentqc" in steps_to_process)):
...
if ("buildbamindex" in steps_to_process) or ("all" in steps_to_process):
	cmd = BuildBamIndex[:]
	cmd.append("I=")
	cmd.append(marked_duplicates_bam)
	cmd.append("O=")
...
indels_vcf_Mills = "/dataVolume/storage/rb_pipeline/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf"
indels_vcf_1000g = "/dataVolume/storage/rb_pipeline/1000G_phase1.indels.hg19.sites.vcf"
if (("baserecalibrator" in steps_to_process) or ("all" in steps_to_process)):
...
if (("printreads" in steps_to_process) or ("all" in steps_to_process)):
	cmd = PrintReads[:]
	cmd.append("-R")
	cmd.append(reference_genome)
	cmd.append("-I")
...
# =====================================================================
#BuildBamIndex I=input.bam 
recal_bam_index= gatk_storage_dir+"recalibrated_indexes/"+r1_base_filename+ "_recalibrated.bai"
...
if "-T" in recal_bam:
	...
	steps_to_process += "skip_mutect2"
if(not os.path.isfile(mutect2_swap_vcf)) and ("skip_mutect2" not in steps_to_process) and("mutect2swap" in steps_to_process):
	cmd  = Mutect2[:]
	cmd.append("-R")
	cmd.append(reference_genome)
...
haplocaller_gvcf = haplocaller_storage_dir+r1_base_filename+"_haplotype.g.vcf"
if "-N_" in recal_bam:
	primary_recal_bam = recal_bam
else:
...
if(not os.path.isfile(selected_vcf)) and ("selectvariants" in steps_to_process):
	cmd = SelectVariants[:]
	cmd.append("-R")
	cmd.append(reference_genome)
	cmd.append("--variant")
...
if(not os.path.isfile(RB_avinput)) and ("annovar" in steps_to_process):
