...
ConvertToAnnovar = ["/media/thor/storage/rb_pipeline/annovar_scripts/convert2annovar.pl"]
GeneAnnotation = ["/media/thor/storage/rb_pipeline/annovar_scripts/annotate_variation.pl"]
FilterAnnotation = ["/media/thor/storage/rb_pipeline/annovar_scripts/table_annovar.pl"] 
CopyWriter = ["/media/thor/storage/rb_pipeline/copywriter/copywriter_cobrinik.R"]
GisticPreProcess = ["/media/thor/storage/rb_pipeline/gistic/gistic_prep.R"]
Gistic = ["/media/thor/storage/rb_pipeline/gistic/run_gistic_cobrinik_rb"]
...
output_directory = "/home/thor/rb_output/"
storage_directory = "/media/thor/storage/rb_pipeline/"
...
mutect2_output_dir=output_directory+"gatk/mutect2/"
...
# =====================================================================
#MarkDuplicates I=input.bam O=marked_duplicates.bam M=marked_dup_metrics.txt
remove_duplicates_bam= picard_output_dir+"removeduplicates/"+ r1_base_filename + "_removed_duplicates.bam"
remove_dup_metrics = picard_output_dir+"removeduplicates/"+r1_base_filename+"_removed_dup_metrics.txt"
picard_tmp_dir = picard_output_dir+"tmp/"
...
marked_duplicates_output_bam= picard_storage_dir+"markduplicates/"+ r1_base_filename + "_marked_duplicates.bam"
hs_metrics= alignment_qc_output_dir+"collecthsmetrics/"+r1_base_filename + "_hsmetrics.txt"
bait_intervals = "/media/thor/storage/rb_pipeline/agilent_baits_hg19.interval_list"
target_intervals = "/media/thor/storage/rb_pipeline/agilent_targets_hg19.interval_list"
if(not os.path.isfile(hs_metrics)) and (("collecthsmetrics" in steps_to_process) or ("all" in steps_to_process)):
	cmd = CollectHsMetrics[:]
	cmd.append("I=")
	cmd.append(marked_duplicates_output_bam)
	cmd.append("O=") 
	...
	cmd.append(target_intervals)
	out_file = alignment_qc_output_dir + r1_base_filename+"_hsmetrics.log"
	err_file = alignment_qc_output_dir + r1_base_filename+"_hsmetrics.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
		 	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
is_metrics= alignment_qc_output_dir+"collectinsertsizemetrics/"+r1_base_filename + "_ismetrics.txt"
is_metrics_histogram= alignment_qc_output_dir+"collectinsertsizemetrics/"+r1_base_filename + "_ismetrics_histogram.pdf"
...
if(not os.path.isfile(is_metrics)) and (("collectismetrics" in steps_to_process) or ("alignmentqc" in steps_to_process)):
	...
	out_file = alignment_qc_output_dir + r1_base_filename+"_ismetrics.log"
	err_file = alignment_qc_output_dir + r1_base_filename+"_ismetrics.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
		 	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
alignsum_metrics= alignment_qc_output_dir+"collectalignmentsummarymetrics/"+r1_base_filename + "_alignsummetrics.txt"
if(not os.path.isfile(alignsum_metrics)) and (("collectalignmentsummarymetrics" in steps_to_process) or ("alignmentqc" in steps_to_process)):
	...
	out_file = alignment_qc_output_dir + r1_base_filename+"_alignment_summary_metrics.log"
	err_file = alignment_qc_output_dir + r1_base_filename+"_alignment_summary_metrics.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
		 	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
if(not os.path.isfile(bam_index)) and ("buildbamindex" in steps_to_process):
	cmd = BuildBamIndex[:]
	cmd.append("I=")
	cmd.append(marked_duplicates_bam)
	cmd.append("O=")
	...
	print " ".join(cmd)
	out_file = picard_output_dir+"markduplicates/" + r1_base_filename+"_buildbamindex.log"
	err_file = picard_output_dir+"markduplicates/" + r1_base_filename+"_buildbamindex.err"
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
                	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
			if(errcode == 0):
...
indels_vcf_Mills = "/media/thor/storage/rb_pipeline/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf"
indels_vcf_1000g = "/media/thor/storage/rb_pipeline/1000G_phase1.indels.hg19.sites.vcf"
...
indels_vcf_dbsnp = "/media/thor/storage/rb_pipeline/dbsnp_138.hg19.excluding_sites_after_129.vcf"
indels_vcf_Mills = "/media/thor/storage/rb_pipeline/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf"
indels_vcf_1000g = "/media/thor/storage/rb_pipeline/1000G_phase1.indels.hg19.sites.vcf"
...
# =====================================================================
#BuildBamIndex I=input.bam 
recal_bam_index= gatk_output_dir+"recalibrated_indexes/"+r1_base_filename+ "_recalibrated.bai"
if(not os.path.isfile(recal_bam_index)) and ("recalibratedbamindex" in steps_to_process):
	cmd = BuildBamIndex[:]
	cmd.append("I=")
	cmd.append(recal_bam)
	cmd.append("O=")
...
#BedToolsCoverage
bedtools_coverage_dir = gatk_output_dir+"bedtoolscoverage/"+r1_base_filename+"/"
bedtools_coverage_metrics = bedtools_coverage_dir+r1_base_filename
bedtools_coverage_bam_list = gatk_output_dir+"depthofcoverage/markduplicates_bams.list"
recalibrated_bam_list = gatk_output_dir+"depthofcoverage/recalibrated_bams.list"
if not os.path.isdir(coverage_metrics_dir):
	subprocess.call(["mkdir", coverage_metrics_dir])
if ("depthofcoverage" in steps_to_process) or ("depthofcoverage" in steps_to_process):
	cmd = DepthOfCoverage[:]
	cmd.append("-R")
	cmd.append(reference_genome)
	cmd.append("-o")
	cmd.append(coverage_metrics)
	cmd.append("-I")
	cmd.append(recal_bam)
	cmd.append("-L")
	cmd.append(target_intervals)
	...
	errcode = subprocess.call(cmd)
	if(errcode == 0):
		print "depthofcoverage finished successfully"
	else:
		print "depthofcoverage failed !!!!"
		del steps_to_process[:]
	del cmd
"""
...
# =====================================================================
#DepthofCoverage
coverage_metrics_dir = gatk_output_dir+"depthofcoverage/"+r1_base_filename+"/"
coverage_metrics = coverage_metrics_dir+r1_base_filename
markduplicates_bam_list = gatk_output_dir+"depthofcoverage/markduplicates_bams.list"
recalibrated_bam_list = gatk_output_dir+"depthofcoverage/recalibrated_bams.list"
if not os.path.isdir(coverage_metrics_dir):
	subprocess.call(["mkdir", coverage_metrics_dir])
if ("depthofcoverage" in steps_to_process) or ("depthofcoverage" in steps_to_process):
...
if(not os.path.isfile(mutect2_vcf)) and ("skip_mutect2" not in steps_to_process) and(("mutect2" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = mutect2_storage_dir + r1_base_filename+"_mutect2.log"
	err_file = mutect2_storage_dir + r1_base_filename+"_mutect2.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
		 	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
# MUTECT2SWAP	
# =====================================================================
mutect2_swap_vcf=mutect2_output_dir+r1_base_filename+"_mutect2_swap.vcf"
...
if(not os.path.isfile(mutect2_swap_vcf)) and ("skip_mutect2" not in steps_to_process) and(("mutect2swap" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = mutect2_output_dir + r1_base_filename+"_mutect2_swap.log"
	err_file = mutect2_output_dir + r1_base_filename+"_mutect2_swap.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
		 	errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
# =====================================================================
mutect_vcf=mutect_output_dir+r1_base_filename+"_mutect.vcf"
mutect_coverage=mutect_output_dir+r1_base_filename+"_mutect_coverage.wig.txt"
regions_bed="/media/thor/storage/rb_pipeline/Agilent_Sureselect_V5_Regions.bed"
if not os.path.isdir(mutect_output_dir):
	subprocess.call(["mkdir", mutect_output_dir])
if "-T" in recal_bam:
	primary_recal_bam = recal_bam
	match_normal_recal_bam = re.sub('-T_1', '-N_1', primary_recal_bam)
elif "-CL" in recal_bam:
...
if "-N_" in recal_bam:
	...
	steps_to_process += "skip haplotypecaller"
if(not os.path.isfile(haplocaller_gvcf)) and ("skip haplotypecaller" not in steps_to_process) and ("haplotypecaller" in steps_to_process):
	cmd  = HaplotypeCaller[:]
	cmd.append("-R")
	cmd.append(reference_genome)
	cmd.append("-I")
...
# GENOTYPEGVCFS
# =====================================================================
pre_filter_all_vcf = haplocaller_output_dir+r1_base_filename+ "_haplotype.vcf"
if(not os.path.isfile(pre_filter_all_vcf)) and ("genotypegvcfs" in steps_to_process):
	gvcf_list = []
	with open('/home/thor/rb_output/gatk/haplocaller/genotype_gvcf_files.txt', 'w') as f:
		for file in os.listdir(haplocaller_output_dir):
			if file.endswith(".g.vcf"):
				f.write(file)
				gvcf_list.append(file)
	cmd = GenotypeGVCFs[:]
	cmd.append("-R")
	cmd.append(reference_genome)
	cmd.append("--variant")
...
RB_vcf = mutect2_output_dir+r1_base_filename+"_filtered.vcf"
...
selected_vcf = mutect2_output_dir+r1_base_filename+"_selected.vcf"
...
RB_variants = annovar_output_dir
...
#	cmd1.append("-withzyg")
	...
	cmd2.append("/media/thor/storage/rb_pipeline/humandb/")
	print " ".join(cmd2)
	...
	cmd3.append("/media/thor/storage/rb_pipeline/humandb/")
...
# COPYWRITER
# =====================================================================
copywriter_input_dir = "/home/thor/rb_output/gatk/"
trio_name = r1_base_filename.rsplit('-', 1)[0]
copywriter_output_dir = "/home/thor/rb_output/copywriter/"+trio_name
...
#GISTIC
# =====================================================================
gistic_output_dir = "/home/thor/rb_output"
gistic_input_dir = "/home/thor/rb_output/copywriter/"+trio_name
if ("gistic" in steps_to_process) or ("all" in steps_to_process):
	#prepare gistic markers file (3 column file output from CopywriteR specifying chromosome, start, and end sites of bins
	cmd1 = GisticPreProcess[:]
	cmd1.append(gistic_input_dir)
	cmd1.append(trio_name)
	print " ".join(cmd1)
	errcode1 = subprocess.call(cmd1)
	if(errcode1 == 0):
		print "gistic finished successfully"
	else:
		print "gistic failed !!!!"
		del steps_to_process[:]
	del cmd1
	cmd2 = Gistic[:]
	cmd2.append(gistic_output_dir)
	cmd2.append(trio_name)
	print " ".join(cmd2)
	errcode2 = subprocess.call(cmd2)
	if(errcode2 == 0):
		print "gistic finished successfully"
