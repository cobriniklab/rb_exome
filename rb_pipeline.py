...
vep = ["/usr/local/bin/TOOLS/ensembl-vep-release-93.5/vep"]
bcftools = ["bcftools", "filter"]
Mutect2_gatk4 = ["gatk", "Mutect2"]
mutect = ["java", "-jar", "/usr/local/bin/TOOLS/mutect/muTect-1.1.4.jar"]
GATK3 = ["java", "-jar", "/usr/local/bin/TOOLS/mutect/muTect-1.1.4.jar"]
...
bamreadcount = ["/usr/local/bin/TOOLS/bam_readcount/bin/bam-readcount"]
luba = ["/usr/local/bin/TOOLS/luba/bin/vcall"]
bafsegmentation = ["/dataVolume/storage/rb_pipeline/src/LOH/baf_estimate_from_bam.sh"]
...
#   gnomad------------------------------
gnomad_exome_reference = "/home/skevin/Homo_sapiens/GNOMAD/gnomad.exomes.r2.0.1.sites.noVEP.vcf.gz"
gnomad_exome_flags = ",gnomADg,vcf,exact,0"
gnomad_exome_reference= gnomad_exome_reference+gnomad_exome_flags
...
target_bed = "./bin/agilent_coverage_files/S04380110_Regions.bed"

if("mosdepth" in steps_to_process):
    cmd = Mosdepth[:]
    # cmd += ["--distribution", coverage_dist]
    cmd += ["--by", target_bed]
    cmd += ["output/"+"mosdepth/"+r1_base_filename]
...
# mutect panel of normals (pon) (gatk 3)
# =====================================================================
mutect_vcf = mutect_output_dir+r1_base_filename+"_bwa_mutect_artifact_detection_mode.vcf"
mutect_log=mutect_output_dir+r1_base_filename+"_mutect_stats.txt"
cosmic_vcf = "/dataVolume/storage/Homo_sapiens/COSMIC/CosmicCodingMuts.vcf.gz"

if not os.path.isdir(mutect_output_dir):
    subprocess.call(["mkdir", mutect_output_dir])
if ((("-N" in recal_bam) or (re.search('\d{3}', recal_bam) is not None)) and ("mutect_pon" in steps_to_process)):
    cmd  = mutect[:]
    cmd += ["-T", "MuTect"]
    cmd += ["-R", reference_genome]
    cmd += ["--dbsnp", indels_vcf_dbsnp]
    # cmd += ["--cosmic", cosmic_vcf]
    cmd += ["-I:tumor", recal_bam]
    cmd += ["--artifact_detection_mode"]
    cmd += ["-o", mutect_log]
    cmd += ["-vcf", mutect_vcf]
    out_file = mutect_output_dir + r1_base_filename+"_mutect.log"
    err_file = mutect_output_dir + r1_base_filename+"_mutect.err"
    print( " ".join(cmd))
...
# =====================================================================
#PrintReads -T=PrintReads -R=reference.fasta -I=input.bam -BQSR= recalibration_report.grp -o=output.bam
selected_vcf = mutect_output_dir+r1_base_filename+"_bwa_mutect_artifact_detection_mode_no_none.vcf"
if ("selectvariants" in steps_to_process):
    cmd = GATK3[:]
    cmd +=["-T", "SelectVariants"]
    cmd += ["-R", reference_genome]
    cmd += ["--exclude_sample_expressions", "none"]
    cmd += ["-V", mutect_vcf]
    cmd += ["-o", selected_vcf]
    out_file = gatk_output_dir + r1_base_filename+"mutect.log"
    err_file = gatk_output_dir + r1_base_filename+"_mutect.err"
    print( " ".join(cmd))
    with open(err_file,"w")as outerr:
        with open(out_file,"w")as outf:
            errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
mutect2_pon_vcf=mutect2_output_dir+"mutect2_pon.vcf.gz"
...
# PREP PANEL OF NORMALS--MUTECT2 (gatk 4)
# =====================================================================
rb_intervals = "bin/rb.bed"
mutect2_pon_vcf=mutect2_output_dir+r1_base_filename+"_pon_mutect2.vcf.gz"
mutect2_pon_bamout=mutect2_output_dir+r1_base_filename+"_mutect2.bamout"

if not os.path.isdir(mutect2_output_dir):
    subprocess.call(["mkdir", mutect2_output_dir])
if (("-N" in recal_bam) and ("mutect2_normal" in steps_to_process)):
    cmd = Mutect2_gatk4[:]
    cmd += ["-R", reference_genome]
    cmd += ["-I", recal_bam]
    cmd += ["-tumor", r1_base_filename]
    ...
    with open(err_file,"w")as outerr:
        with open(out_file,"w")as outf:
            errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
            if(errcode == 0):
                print( "matched normal finished successfully")
            else:
                print( "matched normal failed !!!!")
                del steps_to_process[:]
    del cmd
...
if((not "skip_manta" in steps_to_process) and ("manta" in steps_to_process)):
...
if("filtermutectcalls" in steps_to_process):
    log_file = mutect2_output_dir + r1_base_filename + ".mutect2_filter.log"
    ...
    if(errcode == 0):
        ...
        del steps_to_process[:]
    del cmd
    # with open(filtered_vcf,"wb") as filtered_vcf, open(log_file, "wb") as outerr:
    #   filtermutect2 = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=outerr)
    #   bcftools = subprocess.Popen(["bcftools", "filter", "-i", "FILTER='.'"], stdout=filtered_vcf, stdin=filtermutect2.stdout, stderr=outerr)
    #   filtermutect2.stdout.close()
    #   filtermutect2.wait()
    #   bcftools.wait()
    #   bcftools.communicate()
    # # print datetime.datetime.today()
    # del cmd
...
if("mutect2_vep" in steps_to_process):
    cmd = vep[:]
    cmd += ["--dir", "/dataVolume/storage/vep_cache"]
    cmd += ["--cache"]
    cmd += ["--fasta", "/dataVolume/storage/vep_cache/homo_sapiens/93_GRCh37/Homo_sapiens.GRCh37.75.dna.primary_assembly.fa.gz"]
    cmd += ["--offline"]
    cmd += ["--hgvs"]
    cmd += ["--custom", gnomad_exome_reference]
...
if("strelka_snv_vcf_anno" in steps_to_process):
    cmd = VcfAnno[:]
    cmd += ["-lua", vcfanno_lua]
    cmd += [vcfanno_toml]
    cmd += [strelka_snv_vcf]
...
if("strelka_indel_vcf_anno" in steps_to_process):
    cmd = VcfAnno[:]
    cmd += ["-lua", vcfanno_lua]
    cmd += [vcfanno_toml]
...
# ====================================================================
strelka_vep_unfiltered_vcf = strelka_output_dir+r1_base_filename+"_strelka_snv_unfiltered_vep.vcf"
# strelka_vep_anno_vcf = strelka_output_dir+r1_base_filename+"_strelka_snv_vep.vcf"
strelka_vep_anno_vcf = strelka_vep_unfiltered_vcf
if("strelka_snv_vep" in steps_to_process):
    cmd = vep[:]
    cmd += ["--dir", "/dataVolume/storage/vep_cache"]
    cmd += ["--cache"]
    cmd += ["--fasta", "/dataVolume/storage/vep_cache/homo_sapiens/93_GRCh37/Homo_sapiens.GRCh37.75.dna.primary_assembly.fa.gz"]
...
# ====================================================================
strelka_vep_unfiltered_vcf = strelka_output_dir+r1_base_filename+"_strelka_indel_unfiltered_vep.vcf"
# strelka_vep_anno_vcf = strelka_output_dir+r1_base_filename+"_strelka_indel_vep.vcf"
strelka_vep_anno_vcf = strelka_vep_unfiltered_vcf
if("strelka_indel_vep" in steps_to_process):
    cmd = vep[:]
    cmd += ["--dir", "/dataVolume/storage/vep_cache"]
    cmd += ["--cache"]
    cmd += ["--fasta", "/dataVolume/storage/vep_cache/homo_sapiens/93_GRCh37/Homo_sapiens.GRCh37.75.dna.primary_assembly.fa.gz"]
...
if("bafsegmentation" in steps_to_process):
    ...
    # cmd += ["-f", """'%CHROM %POS %FILTER\n'"""]
    print( " ".join(cmd))
    log_file = baf_output_dir +r1_base_filename+"_bafsegmentation.log"
    err_file = baf_output_dir +r1_base_filename+"_bafsegmentation.err"
...
# run bam-readcount on all files
# ====================================================================
bamreadcount_output_dir = output_directory+"bamreadcount/"
double_check_bed = "/dataVolume/storage/rb_pipeline/results/SNV/vc_somatic_variants_for_luba.bed"
if not os.path.isdir(bamreadcount_output_dir):
    subprocess.call(["mkdir", bamreadcount_output_dir])
bamreadcount_txt = bamreadcount_output_dir+r1_base_filename+"_all_readcount.txt"
if("bamreadcount" in steps_to_process):
    cmd = bamreadcount[:]
    cmd += ["-l", double_check_bed]
    cmd += ["-f", reference_genome]
    cmd += [recal_bam]
    print( " ".join(cmd))
    err_file = bamreadcount_output_dir + r1_base_filename + "_bamreadcount.err"
    out_file = bamreadcount_txt
    print(out_file)
    with open(err_file,"w")as outerr:
        with open(out_file,"w")as outf:
            errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
            if(errcode == 0):
                print( "bamreadcount finished successfully")
            else:
                print( "bamreadcount failed !!!!")
                del steps_to_process[:]
    del cmd

# run luba on all files
# ====================================================================
luba_output_dir = output_directory+"luba/"
double_check_bed = "/dataVolume/storage/rb_pipeline/results/SNV/vc_somatic_variants_for_luba.bed"
if not os.path.isdir(luba_output_dir):
    subprocess.call(["mkdir", luba_output_dir])
luba_txt = luba_output_dir+r1_base_filename+"_readcount.txt"
if("luba" in steps_to_process):
    cmd = luba[:]
    cmd += ["-m", "pileup"]
    cmd += ["-b", recal_bam]
    cmd += ["-r", double_check_bed]
    cmd += ["-f", reference_genome]
    print( " ".join(cmd))
    err_file = luba_output_dir + r1_base_filename + "_luba.err"
    out_file = luba_txt
    print(out_file)
    with open(err_file,"w")as outerr:
        with open(out_file,"w")as outf:
            errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
            if(errcode == 0):
                print( "luba finished successfully")
            else:
                print( "luba failed !!!!")
                del steps_to_process[:]
    del cmd
