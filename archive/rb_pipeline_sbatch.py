...
ConvertToAnnovar = ["/dataVolume/storage/rb_pipeline/annovar_scripts/convert2annovar.pl"]
GeneAnnotation = ["/dataVolume/storage/rb_pipeline/annovar_scripts/annotate_variation.pl"]
FilterAnnotation = ["/dataVolume/storage/rb_pipeline/annovar_scripts/table_annovar.pl"] 
CopyWriter = ["/dataVolume/storage/rb_pipeline/copywriter/copywriter_cobrinik.R"]
GisticPreProcess = ["/dataVolume/storage/rb_pipeline/gistic/gistic_prep.R"]
Gistic = ["/dataVolume/storage/rb_pipeline/gistic/run_gistic_cobrinik_rb"]
ValidateVariants = ["java", "-jar", "/usr/share/java/GenomeAnalysisTK.jar", "-T", "ValidateVariants"]
CombineVariants = ["java", "-jar", "/usr/share/java/GenomeAnalysisTK.jar", "-T", "CombineVariants"]
min_base_quality = "15"
min_read_length  = "25"
...
if(not os.path.isfile(remove_duplicates_bam)) and (("removeduplicates" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = picard_output_dir + r1_base_filename+"_removeduplicates.log"
        err_file = picard_output_dir + r1_base_filename+"_removeduplicates.err"
        print " ".join(cmd)
        with open(err_file,"w")as outerr:
                with open(out_file,"w")as outf:
                        errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
                        if(errcode == 0):
                                print "removeduplicates finished successfully"
                        else:
...
if ("bgzip" in steps_to_process) or ("all" in steps_to_process):
        cmd = bgzip[:]
        cmd.append(mutect2_vcf) 
        ...
        if(errcode == 0):
                ...
                print "copywriter for sample"+r1_base_filename+" failed !!!!"
                del steps_to_process[:]
        del cmd
...
if ("tabix" in steps_to_process) or ("all" in steps_to_process):
        cmd = tabix[:]
        cmd.append(mutect2_vcf)
        cmd.append(trio_name)
        cmd.append(copywriter_output_dir)
...
# COPYWRITER
# =====================================================================
copywriter_input_dir = "/dataVolume/storage/rb_pipeline/picard/removeduplicates"
...
if ("copywriter" in steps_to_process) or ("all" in steps_to_process):
	...
	cmd.append(copywriter_output_dir)
#        out_file = copywriter_output_dir+ "_copywriter.log"
#        err_file = copywriter_output_dir+"_copywriter.err"
        print " ".join(cmd)
#        with open(err_file,"w")as outerr:
#                with open(out_file,"w")as outf:
#                        errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
#GISTIC
# =====================================================================
gistic_output_dir = "/dataVolume/storage/rb_pipeline/gistic"
gistic_input_dir = "/dataVolume/storage/rb_pipeline/copywriter/"+r1_base_filename
...
#VALIDATEVARIANTS
# =====================================================================
all_vcf = mutect2_output_dir+"all.vcf"
if("validatevariants" in steps_to_process) or ("all" in steps_to_process):
        cmd = ValidateVariants[:]
        cmd.append("-R")
        cmd.append(reference_genome)
        cmd.append("-V")
        cmd.append(mutect2_vcf)
	cmd.append("--validationTypeToExclude")
	cmd.append("ALL")
	out_file = mutect2_output_dir + r1_base_filename+ "validate_variants.log"
	err_file = mutect2_output_dir + r1_base_filename+"validate_variants.err"
        print " ".join(cmd)
        with open(err_file,"w")as outerr:
                with open(out_file,"w")as outf:
                        errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
                        if(errcode == 0):
                                print "validatevariants finished successfully"
                        else:
                                print "validatevariants failed !!!!"
                                del steps_to_process[:]
        del cmd
...
# =====================================================================
all_vcf = mutect2_output_dir+"all.vcf"
if(not os.path.isfile(all_vcf)) and (("combinevariants" in steps_to_process) or ("all" in steps_to_process)):
        with open('/dataVolume/storage/rb_pipeline/gatk/mutect2/vcf_list.txt', 'r+') as f:
                for file in os.listdir(mutect2_output_dir):
                        if file.endswith("_1_mutect2.vcf.gz"):
                                print(file)
	...
	cmd.append(reference_genome)
 	for i in vcf_list:
        ...
        cmd.append("-o")
	cmd.append(all_vcf)
	cmd.append("-genotypeMergeOptions")
	cmd.append("UNIQUIFY")
        print " ".join(cmd)
        errcode = subprocess.call(cmd)
        if(errcode == 0):
                print "combinevariants finished successfully"
        else:
                print "combinevariants failed !!!!"
                del steps_to_process[:]
        del cmd
